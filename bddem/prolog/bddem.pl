 ...
 *
 * Initializes an environment data structure for storing a BDD.
 ...
 *
 * Terminates the environment data structure for storing a BDD.
...
/**
 * add_opt_var(++Environment:int,++ProbabilityDistribution:list, ++Rule:int,-Variable:int) is det.
 *
 * Returns in Variable the index of a new random variable to optimize in Environment with
 * NumberOHeads values and probability distribution ProbabilityDistribution which is its initial value.
 */
...
ret_prob_infl_c(Env,(_,BDD),P,T):-
  ret_influence_prob(Env,BDD,P,T).

ret_prob_paths_c(Env,(_,BDD),P,[TPaths,TProbs]):-
  ret_paths_prob(Env,BDD,P,TPaths,TProbs).

%ret_opt_prob_c(Env,(_,BDD),Objective,GoalA,ConstraintsList,Bounds,LIndexName,Algorithm,MaxExecTime,Assigments):-
%  ret_opt_prob(Env,BDD,Objective,GoalA,ConstraintsList,Bounds,LIndexName,Algorithm,MaxExecTime,Assigments).

convert_numbers([],[]).
convert_numbers([H|T],[HN|TN]):-
  ( atom_number(H,HN) ->  true ; H = HN ),
  convert_numbers(T,TN).

expression_to_list(Expression,LO):-
  term_to_atom(Expression,C),
  atom_chars(C,LC),
  convert_numbers(LC,LO).

ret_prob_constr_c(Env,(_,BDD),[Objective],_Mode,ConstraintsList,Assigments):-
  expression_to_list(Objective,LO),
  maplist(expression_to_list,ConstraintsList,CL),
  writeln(LO),
  writeln(CL),
  ret_prob_constr(Env,BDD,LO,0,CL,Assigments).
...
ret_justs_bdd(Env,BDD,LIndexName,SymbolicEquation):-
  ret_justs_bdd(Env,BDD,LIndexName,0,SymbolicEquation). % <- qui chiamo il predicato/funzione in c

% ret_justs_bdd(Env,BDD,LIndexName,EOL,SymbolicEquation):-
%  ret_justs_bdd(Env,BDD,LIndexName,EOL,SymbolicEquation). % <- qui chiamo il predicato/funzione in c